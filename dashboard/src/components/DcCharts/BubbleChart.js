...
const groupRemoveRudcer = (point, v) => {
  const p = point;
  p.count -= 1;
  p.absGain -= v.close - v.open;
  p.fluctuation -= Math.abs(v.close - v.open);
...
// Divref is a refere3nce to the div we're attaching to
const bubbleChartFunc = (divRef, data) => {
  const yearlyDimension = data.dimension(d => timeYear(d.dd).getFullYear());
  const yearlyPerformanceGroup = yearlyDimension
    .group()
    .reduce(groupAddReducer, groupRemoveRudcer, groupInitalizer);
  const yearlyBubbleChart = dc.bubbleChart(divRef);
